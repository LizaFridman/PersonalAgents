...
4. **Authoritative card data** — for anything the curated rules don't cover (specific card rulings, current oracle text, legality, a card you don't already know well):
   - **Check `wiki/card-cache.md` first.** If the card's static facts (color identity, mana cost, type, oracle text) are already listed there, trust them — no fetch needed. Prices, legality, and EDHREC synergy are never cached, regardless of what's in the card cache — always fetch those live.
