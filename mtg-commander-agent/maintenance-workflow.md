...
Note that the standard way to update a wiki file's *content* (per `wiki-structure.md`) is itself a create-new-then-trash-old + repoint-fileIds swap, since the Drive tools can't rewrite contents in place. Done in one session with the verify-single-copy step it's safe; the failure mode above is a concurrent or half-finished swap that leaves the old file, or a duplicate, behind.
