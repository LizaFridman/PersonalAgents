...
For "what's this worth" questions, fetch the card from Scryfall (`https://api.scryfall.com/cards/named?fuzzy=<name>`) and read its `prices` object: `usd`, `usd_foil`, `eur`, `tix`. Report the relevant one(s) given the card's actual finish (check owned quantity/foil status from the collection CSV when the question is about an owned card). Don't log anything to `price_log.csv` unless asked to — on-demand lookups are ephemeral.

**Pacing**: Scryfall's public API has no batch endpoint available to this agent (see `project-instructions.md`), and asks for sustained traffic to stay under ~10 requests/second. That's not a practical constraint for a handful of cards in a chat, but for a large batch (e.g. "price my whole 100-card deck" or a big chunk of the collection):
